...
Meanwhile, `pml.quiz` contains 20 samples whose labels are not given. They are the 20 prediction problems on Coursera quiz 4. There is no `classe` variable and the last column is named `problem_id`. 
...
For random forest, I use the `parRF` method to make it run on 4 parallel cores. In addition, `prox = F` and `nodesize = 10` are specified so as to reduce the amount of computation and speed up the method. The `x = ...` and `y = ...` are also supposed to make `train()` run faster.
